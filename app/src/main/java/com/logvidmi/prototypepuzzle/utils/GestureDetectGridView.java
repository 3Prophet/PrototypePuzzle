...
public class GestureDetectGridView extends GridView {
    ...
    private void init(final Context context) {
        gDetector = new GestureDetector(context, new GestureDetector.SimpleOnGestureListener() {
            ...
            StartGame startGameInst = (StartGame) context;
            ...
            public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
                ...
                if (Math.abs(e1.getY() - e2.getY()) > SWIPE_MAX_OFF_PATH) {
                    ...
                    if (e1.getY() - e2.getY() > SWIPE_MIN_DISTANCE) {
                        startGameInst.moveTiles(context, startGameInst.up, position);
                    } else if (e2.getY() - e1.getY() > SWIPE_MIN_DISTANCE) {
                        startGameInst.moveTiles(context, startGameInst.down, position);
                    }
                } else {
                    ...
                    if (e1.getX() - e2.getX() > SWIPE_MIN_DISTANCE) {
                        startGameInst.moveTiles(context, startGameInst.left, position);
                    } else if (e2.getX() - e1.getX() > SWIPE_MIN_DISTANCE) {
                        startGameInst.moveTiles(context, startGameInst.right, position);
                    }
                }
